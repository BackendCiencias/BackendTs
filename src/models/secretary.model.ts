...
secretarySchema.methods.encryptPassword = async(password: string) : Promise<string> => {
  const salt = await  bcrypt.genSalt(10);
  return bcrypt.hash(password, salt);
};

secretarySchema.methods.validatePassword = async function(password: string):Promise<boolean>{
